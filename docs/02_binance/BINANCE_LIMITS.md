...
> **Verified:** 2026-01-29 against Binance Futures API docs.

### 6.1 Endpoint Weights (Futures USDT-M)

| Endpoint | Method | Path | IP Weight | Order Count | Notes |
|----------|--------|------|-----------|-------------|-------|
| Place order | POST | `/fapi/v1/order` | 0 | 1 | Single limit/market order |
| Cancel order | DELETE | `/fapi/v1/order` | 1 | 0 | By orderId or clientOrderId |
| Cancel all | DELETE | `/fapi/v1/allOpenOrders` | 1 | 0 | Per symbol |
| Query order | GET | `/fapi/v1/order` | 1 | 0 | |
| Open orders | GET | `/fapi/v1/openOrders` | 1/5 | 0 | 1 with symbol, 5 without |
| Position risk | GET | `/fapi/v2/positionRisk` | 5 | 0 | |
| Account info | GET | `/fapi/v2/account` | 5 | 0 | |
| User trades | GET | `/fapi/v1/userTrades` | 5 | 0 | |
| Listen key | POST/PUT/DELETE | `/fapi/v1/listenKey` | 1 | 0 | Create/keepalive/close |

> **Weight columns:** "IP Weight" counts against `X-MBX-USED-WEIGHT-1M` (2400/min limit). "Order Count" counts against `X-MBX-ORDER-COUNT-10S/1M` (300/10s, 1200/min limits).
...
For vol harvesting use case (max 3 symbols x 6 levels):

```
Worst case full requote: 3 x 6 cancel + 3 x 6 place = 36 orders
...
| -2011 | Unknown order (CANCEL_REJECTED) | No action (normal race) | Check if fill occurred |
| -1015 | Too many new orders | FREEZE 1min | Reduce requote frequency |
| -2022 | ReduceOnly order rejected | No action | Position already flat or conflicting orders, ignore |
| -4131 | PERCENT_PRICE filter limit | No action | Price volatility protection, retry with better price |
| -1013 | Invalid message / filter failure | No action | Refresh SymbolSpec -> re-sanitize |
| -1111 | Precision too high (BAD_PRECISION) | No action | Refresh SymbolSpec -> adjust rounding |
...
> **Verification status:** [OK] Verified 2026-01-29
> - Endpoints/weights verified against [Binance Futures Trade API](https://developers.binance.com/docs/derivatives/usds-margined-futures/trade/rest-api/New-Order)
> - Rate limits verified against [Rate Limits FAQ](https://www.binance.com/en/support/faq/rate-limits-on-binance-futures-281596e222414cdd9051664ea621cdc3)
> - Error codes verified against [Error Code docs](https://developers.binance.com/docs/derivatives/usds-margined-futures/error-code)
>
> **Runtime verification (must implement):**
> 1. Log used-weight headers from every REST response (`X-MBX-USED-WEIGHT-*`, `X-MBX-ORDER-COUNT-*`)
> 2. Parse `Retry-After` header on 429 responses
> 3. Alert if usage exceeds 80% of limits
